      SUBROUTINE DASYMTX( AAD, EVECD, EVALD, M, IA, IEVEC, IER, WKD )
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DABS, MIN, DSIGN, DSQRT
c     ..
...
c                           ** Handle 1x1 and 2x2 special cases
      ...
      ELSE IF( M.EQ.2 ) THEN
         ...
         DISCRI = ( AAD( 1,1 ) - AAD( 2,2 ) )**2 + 4.*AAD( 1,2 )*AAD( 2,1 )
...
C    &       CALL ERRMSG( 'ASYMTX--complex evals in 2x2 case',.TRUE. )
         ...
         EVALD( 1 ) = 0.5*( AAD( 1,1 ) + AAD( 2,2 ) + SGN*DSQRT( DISCRI ) )
         EVALD( 2 ) = 0.5*( AAD( 1,1 ) + AAD( 2,2 ) - SGN*DSQRT( DISCRI ) )
         EVECD( 1,1 ) = 1.0
         EVECD( 2,2 ) = 1.0

         IF( AAD( 1,1 ) .EQ. AAD( 2,2 ) .AND.
     &       ( AAD( 2,1 ).EQ.0.0 .OR. AAD( 1,2 ).EQ.0.0 ) ) THEN
...
c                          ** Look for single small sub-diagonal element
  410 CONTINUE
      ...
      DO 420 I = L, N
         ...
         IF( DABS( AAD( LB, LB-1 ) ).LE. TOL*S ) GO TO  430
