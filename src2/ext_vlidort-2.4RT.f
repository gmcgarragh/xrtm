...
c*******************************************************************************
     ...
     &      derivs, epsilon, info, n_quad2)
      ...
      real*8 I_p(n_quad2*n_stokes, n_phis, n_ulevels)
      real*8 I_m(n_quad2*n_stokes, n_phis, n_ulevels)

      real*8 K_p(n_quad2*n_stokes, n_phis, n_derivs, n_ulevels)
      real*8 K_m(n_quad2*n_stokes, n_phis, n_derivs, n_ulevels)
...
c     ******************************************************************
      ...
      character*10 kernel_names(max_kernels_lidort + 5)
      data kernel_names /
     &     'Lambertian',
     &     'Ross-thin ',
     &     'Ross-thick',
...
c     double precision sza_local_input(0:maxlayers, maxbeams)
      ...
      a = 180.d0 / pi
      if (quad_output .ne. 0) then
           n_user_vzangles = n_quad
           do i = 1, n_quad
                quad_index(i) = n_quad - i + 1
                user_vzangles(i) = acos(qx(quad_index(i))) * a
           enddo
      else
           if (n_umus .eq. 0) then
                n_user_vzangles = n_quad
                ...
                do i = 1, n_umus
                     user_vzangles(i) = angles(quad_index(i))
                enddo
           endif
      endif

      n_user_levels = n_ulevels
      if (utau_output .eq. 0) then
           do i = 1, n_ulevels
                user_levels(i) = ulevels(i)
           enddo
      else
           write (0, *) 'ERROR: no utau input for vlidort'
           info = 1
           return
           a = utaus(1)
c          do i = 1, n_ulevels
...
c     integer finegrid(maxlayers)
      ...
      if (n_kernels == 1 .and. kernels(1) == 0) then
           ...
           write (0, *) 'ERROR: vlidort-2.4RT only supports a lambertian surface'
           info = 1
           return
...
c     integer n_brdf_stokessq
      ...
      if (thermal .eq. 0) then
           ...
           n_thermal_coeffs          = 1
           do i = 0, n_layers
                thermal_bb_input(i)  = levels_b(i+1)
           enddo
...
c          character*31 profilewf_names(max_atmoswfs)
c          character*31 columnwf_names (max_atmoswfs)
c          character*22 surfacewf_names(max_surfacewfs)
      endif
...
c     ******************************************************************
      if (n_derivs .eq. 0) then
           call vlidort_master_lambertian(status_inputcheck,
     &                                    status_calculation)
           if (status_inputcheck .ne. 0) then
                write (0, *) 'ERROR: vlidort_master_lambertian(), '//
     ...
     &                                      status_calculation)
           if (status_inputcheck .ne. 0) then
                write (0, *) 'ERROR: vlidort_l_master_lambertian(), '//
     &                   'status_inputcheck = ', status_inputcheck
                info = 1
                return
           endif
...
c     ******************************************************************
      flag = .false.
      if ((nstokes .eq. 1 .and. (n_user_vzangles .eq. 1 .and.
     &     user_vzangles(1) .eq. zero) .and. (.not. do_ssfull .or.
           ...
           do i = 1, n_user_vzangles
                do j = 1, n_stokes
                     ...
                     do k = 1, n_phis
                          ...
                          do l = 1, n_user_levels
                               I_p(i1, k, l) = stokes(l, i2, j, 1)
                               I_m(i1, k, l) = stokes(l, i2, j, 2)
                          enddo
                     enddo
                enddo
           enddo
                          ...
                          do i = 1, n_user_vzangles
                               do j = 1, n_stokes
                                    ...
                                    do k = 1, n_phis
                                         ...
                                         do m = 1, n_user_levels
                                              K_p(i1, k, l, m) =
     &                                             profilewf(i_deriv, i_layer, m, i2, j, 1)
                                              K_m(i1, k, l, m) =
     &                                             profilewf(i_deriv, i_layer, m, i2, j, 2)
                                         enddo
                                    enddo
                               enddo
                          enddo
     ...
     &          (n_derivs, n_layers, n_user_levels, n_totalatmos_wfs,
...
c***********************************************************************
     ...
     &     (n_derivs, n_user_levels, n_totalatmos_wfs, deriv_type,
     ...
     *      mean_p_l, mean_m_l)
      ...
      real*8 lidort_mean(MAX_USER_LEVELS, MAX_SZANGLES,
     &                   MAXSTOKES, MAX_DIRECTIONS)
      real*8 lidort_atmos_l  (MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS,
     &                        MAX_SZANGLES, MAXSTOKES, MAX_DIRECTIONS)
      real*8 lidort_surface_l(MAX_SURFACEWFS, MAX_USER_LEVELS,
     &                        MAX_SZANGLES, MAXSTOKES, MAX_DIRECTIONS)

      real*8 mean_p(n_user_levels)
      real*8 mean_m(n_user_levels)

      real*8 mean_p_l(n_derivs, n_user_levels)
      real*8 mean_m_l(n_derivs, n_user_levels)
